...
async fn get_replicas() -> impl Responder {
    match MessageBus::get_replicas(Filter::All).await {
        Ok(replicas) => HttpResponse::Ok().json(replicas),
...
async fn get_replica(
    ...
    match MessageBus::get_replica(Filter::Replica(replica_id)).await {
        Ok(replicas) => HttpResponse::Ok().json(replicas),
        Err(error) => (RestError::from(error)).into(),
    }
}
...
async fn get_node_replicas(
    ...
    match MessageBus::get_replicas(Filter::Node(node_id)).await {
        Ok(replicas) => HttpResponse::Ok().json(replicas),
        Err(error) => (RestError::from(error)).into(),
    }
}
...
async fn get_node_pool_replicas(
    ...
    match MessageBus::get_replicas(Filter::NodePool(node_id, pool_id)).await {
        Ok(replicas) => HttpResponse::Ok().json(replicas),
        Err(error) => (RestError::from(error)).into(),
    }
}
...
async fn get_node_pool_replica(
    ...
    match MessageBus::get_replica(Filter::NodePoolReplica(
        node_id, pool_id, replica_id,
    ))
    .await
    {
        Ok(replica) => HttpResponse::Ok().json(replica),
        Err(error) => (RestError::from(error)).into(),
    }
}
...
async fn put_replica(
    ...
    match MessageBus::create_replica(create).await {
        Ok(replica) => HttpResponse::Ok().json(replica),
        Err(error) => (RestError::from(error)).into(),
    }
}
...
async fn destroy_replica(filter: Filter) -> impl Responder {
    ...
    match MessageBus::destroy_replica(destroy).await {
...
async fn share_replica(filter: Filter, protocol: Protocol) -> impl Responder {
    ...
    match MessageBus::share_replica(share).await {
...
async fn unshare_replica(filter: Filter) -> impl Responder {
    ...
    match MessageBus::unshare_replica(unshare).await {
        Ok(_) => HttpResponse::Ok().json(()),
        Err(error) => (RestError::from(error)).into(),
    }
}
