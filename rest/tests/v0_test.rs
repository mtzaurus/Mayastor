...
async fn client_test(mayastor: &str, test: &ComposeTest) {
    ...
    info!("Nodes: {:#?}", nodes);
    let _ = client.get_pools(Filter::All).await.unwrap();
    ...
    assert_eq!(
        Some(&pool),
        client.get_pools(Filter::All).await.unwrap().first()
    );
    let _ = client.get_replicas(Filter::All).await.unwrap();
    let replica = client
    ...
    assert_eq!(
        Some(&replica),
        client.get_replicas(Filter::All).await.unwrap().first()
    );
    client
        ...
        .await
        .unwrap();
    assert!(client.get_replicas(Filter::All).await.unwrap().is_empty());

    let nexuses = client.get_nexuses(Filter::All).await.unwrap();
    ...
    client
        ...
        .await
        .unwrap();
    assert!(client.get_nexuses(Filter::All).await.unwrap().is_empty());
