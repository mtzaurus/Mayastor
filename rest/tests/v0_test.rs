...
async fn client_test(
    ...
    info!("Nodes: {:#?}", nodes);
    let _ = client.get_pools(Filter::All).await?;
    ...
    assert_eq!(Some(&pool), client.get_pools(Filter::All).await?.first());
    let _ = client.get_replicas(Filter::All).await?;
    let replica = client
    ...
    assert_eq!(
        Some(&replica),
        client.get_replicas(Filter::All).await?.first()
    );
    client
        ...
        .await?;
    assert_eq!(client.get_replicas(Filter::All).await?.is_empty(), true);
    client
        ...
        .await?;
    assert_eq!(client.get_pools(Filter::All).await?.is_empty(), true);
